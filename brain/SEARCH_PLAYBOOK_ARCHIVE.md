# Search playbook archive

## Before 2026-09-05 R3 consolidation
...
- 2026-09-05: Source-constraining improves buyer-language quality but does not automatically create a business. Facebook-visible snippets and Freelancer.gr surfaced real pay asks: paid Greek-language practice, pet-food forwarding, contract takeover, and an Excel list of Greek therapy/health centers. Treat these as fragments unless at least several same-job asks repeat in one reachable channel.
- 2026-09-05: Deliverable-first verbs are necessary but not sufficient. Broad Google queries for `στείλε μου λίστα`, `θέλω λίστα προμηθευτών`, `να μου κάνει έρευνα αγοράς`, `να αναλάβει αρχείο` mostly returned official registries, official price lists, agency/service SEO, tax/audit articles, reports and existing providers. Next time source-constrain first: `site:facebook.com/groups`, `site:insomnia.gr`, `site:freelancer.gr`, `site:car.gr`, forums/classifieds/job boards plus pay verbs (`θα πληρώσω`, `πόσο`, `αναλαμβάνει`, `ψάχνω κάποιον`).
...
- 2026-09-04: Refusal language is higher signal than residue language, but it must be current. `λογιστής δεν το αναλαμβάνει myDATA` found the exact shape in an old Insomnia thread; do not revive a 2026 idea from a 2021 quote unless fresh posts repeat it near the live deadline.
- 2026-09-04: Post-choice is not automatically open. `Skroutz 0 διαθέσιμο`, `λάθη myDATA`, `παραλείψεις/αποκλίσεις` found real residue, but partner support, ERP/vendor docs, AADE support and specialists followed the problem downstream. Search for refusal/delay language (`δεν απαντάει`, `δεν το αναλαμβάνει`, `μου λέει να το κάνω μόνος`) before naming a post-choice helper.
...
- 2026-09-04: For exhibitor targets, event+country queries are weaker than event+hall+stand+contact. `SMM Hamburg 2026 Greece Hall stand contact` and `Automechanika Frankfurt 2026 Greece Hall Stand contact` surfaced Valiadis, Microtem, DNA Filters, Recor, AutoGS/Feral and Sakkos with actual outreach handles.
...
- 2026-08-24: For investor-tax routing, search exact pain artifacts (`029`, `743`, `VAT broker`, `annual statement`, `UCITS`, `dividends`, `Trade Republic`, `Revolut`) because they separate real handoff prep from generic "ask an accountant" chatter.
- 2026-08-24: Candidate supply is not a bench. A page like AT Office's Crypto/ETF 2026 guide proves who to call first, not that the finder has solved supply. Do not upgrade beyond probation without direct office confirmation.
...
- 2026-07-03: For Ferry Claims, validate only prep/translation/organization, not legal representation or guaranteed compensation. The free eligibility check is the hook; paid intent is for a prepared claim pack, not legal advice.
...
## What Works
- Greek-first for laws, prices and complaints; English only for nomad, tourist, diaspora, or app-store/global competitor checks.
- Add the year plus law/deadline words when researching regulation.
- `site:taxheaven.gr` is high-signal for tax/e-invoicing timing.
- Search official deadline source + pricing + social truth for compliance ideas. Example: `site:taxheaven.gr ilektroniki timologisi 2026`, then `myDATA timi`, then reddit/social complaint search.
- For marketplaces, search app names plus local intent. App visibility is a competitor signal even before local market share is proven.

## What Wastes Tokens
- Vague English "Greece business opportunity" searches.
- Fetching pages when snippets already answer timing, price or competitor existence.
- Broad competitor-landscape searches before one exact "already exists?" query.
- Treating a free government compliance tool as irrelevant. It changes the business model.

## Source Signal Ratings
- taxheaven.gr / official tax snippets - HIGH for regulation and timing.
- Greek vendor pricing pages - MED/HIGH for price discovery, but biased toward paid solutions.
- Reddit/social snippets - HIGH for pain language, LOW for market size.
- App stores / global marketplace pages - MED for competitor existence, LOW for Greek supply density unless local listings are visible.
...
- 2026-07-03: Once a survivor enters the pool, the next pass should attack its pay-intent gate or run an orthogonal guardrail around it. Do not keep reopening adjacent dead lanes if the repo already records the kill.
